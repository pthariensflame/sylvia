...
import com.pthariensflame.sylvia.util.RoseBush.NonEmpty.Leaf
import com.pthariensflame.sylvia.util.RoseBush.NonEmpty.Nested
import com.pthariensflame.sylvia.util.RoseBush.Visitor.Companion.visit
...
sealed class RoseBush<out Elem, out Gap> {
    companion object {
        ...
        inline operator fun <Elem, Gap> invoke(
            ...
            val builderScope =
                BuilderScope<Elem, Gap>(estimatedSize)
        ...
        inline fun <Elem, Gap> Triple<RoseBush<Elem, Gap>?, Gap, RoseBush<Elem, Gap>?>?.concatOrEmpty(): RoseBush<Elem, Gap> =
            this?.let { concat(first.orEmpty(), second, third.orEmpty()) } ?: empty()
    }
    ...
    interface Visitor<in Elem, in Gap> {
        companion object {
            @Contract(pure = true)
            @JvmStatic
            @Suppress("NOTHING_TO_INLINE")
            inline fun <Elem, Gap> Visitor<Elem, Gap>.visit(
        ...
        fun visitLeaf(value: Elem)

        fun visitGap(gap: Gap)
    ...
    private object Empty : RoseBush<Nothing, Nothing>() {
        ...
        override fun acceptVisitor(visitor: Visitor<Nothing, Nothing>): Unit =
            visitor.visitEmpty()
    ...
    @ValueType
    private sealed class NonEmpty<out Elem, out Gap> : RoseBush<Elem, Gap>() {
        final override fun isEmpty(): Boolean = false

        @ValueType
        data class Leaf<out Elem>(
            @JvmField val _value: Elem,
        ) : NonEmpty<Elem, Nothing>() {
            ...
            override fun acceptVisitor(visitor: Visitor<Elem, Nothing>): Unit =
        ...
        @ValueType
        data class Nested<out Elem, out Gap>(
            ...
            override fun acceptVisitor(visitor: Visitor<Elem, Gap>) {
                visitor.enterNested()
                for ((c, g) in _children.asSequence().zip(_childGaps.asSequence())) {
                    visitor.visit(c)
                    visitor.visitGap(g)
                }
